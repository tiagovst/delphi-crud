...
begin
  if DmPessoa.sqlPes.Active then
  begin
    Result := DmPessoa.sqlPes.FieldByName('codigo').AsInteger;
  end
  else
  begin
...
function TPessoaController.sqlAll: TFDQuery;
begin
  Result := DmPessoa.sqlAll;
end;

function TPessoaController.sqlPes: TFDQuery;
begin
  Result := DmPessoa.sqlPes;
end;
